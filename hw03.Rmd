...
CLT_CI <- function( data, alpha=0.05) {
  ...
  m <- estimate_pois_mean(data)
  v <- estimate_pois_var(data)
  ...
  upper <- m + (1.96*(v/length(data))^0.5)
  lower <- m - (1.96*(v/length(data))^0.5)
...
simulation_CI <- function( data, alpha=0.05) {
  # TODO: code goes here.
  m <- estimate_pois_mean(data)
  replicates <- rep(NA,400); # We will store replicates of lambdahat here.
  for ( i in 1:400) {
    fake_data <- rpois(n=length(data), lambda=m);
    replicates[i] <- mean( fake_data );
  }
 CI <- quantile( replicates, probs=c(0 + (alpha/2), 1 - (alpha/2)), names=FALSE );
...
```{r}
exploring <- function(l, a, n) {
  replicates_1 <- rep(NA, 20)
  for (i in 1:20) {
    temp <- estimate_coverage(CLT_CI, 400, n, lambdatrue = l, alpha = a)
    replicates_1[i] = temp
  }
  replicates_2 <- rep(NA, 20)
  for (i in 1:20) {
    temp <- estimate_coverage(simulation_CI, 400, n, lambdatrue = l, alpha = a)
    replicates_2[i] = temp
  }
  return(c(mean(replicates_1), mean(replicates_2)))
}

exploring(0.6, 0.05, 30)
```

Consistently, the CLT seems to be performing better than the simulation method. Both of them seem to be doing very well but the CLT is always a little better than the simulation based method. 
...
lambdahat <- 0.7; # TODO: write code; store your estimate in lambdahat.
```

I chose this because the mean of the deaths varibale in the data is 0.7. Choosing 0.7 for the lambda in the rpois gets us data with the mean being close to 0.7.
...
```{r}
n <- 280
data <- rpois(n, lambdahat)
l <- mean(data);
replicates <- rep(NA,400); # We will store replicates of lambdahat here.
for ( i in 1:400) {
  fake_data <- rpois(n=n, lambda=l);
  replicates[i] <- mean( fake_data );
}
...
First, I am estimating our distribution from the data. Then, I am generating the data with the parameter equal to the estimate from part a. I, then, repeat the previous step many times, computing a new estimate on each new data sample(Monte Carlo). We can use that to construct a confidence interval for λ, by looking at the quantiles of our sample. This CI gives other reasonable values that our estimate might have been.
...
I did some research online and I found out that you can do something called a "dispersion test" to see if the data is following the Poisson distribution. This test relies on the fact that the Poisson distribution's mean is equal to its variance, and the the ratio of the variance to the mean in a sample of n counts from a Poisson distribution should follow a Chi-square distribution with n-1 degrees of freedom.
...
dispersion_test(mule_kicks$deaths)
```

As you can see from value of the dispersion test, the assumption that the data is following the Poisson distribution is not reasonable. 
***

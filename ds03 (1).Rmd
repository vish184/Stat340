...
# 
 ratings.standard = read_csv("chess.csv")
...
# 
 ratings.standard.in %>% 
   group_by(Sex) %>% 
   summarise(num  = n(), mean = mean(Rating), max = max(Rating))  # for help, see dplyr.tidyverse.org/reference/n.html
#             mean = ... ,
#             max  = ... )
```
...
### do 2.a here
data(mtcars);
mtcars.small <- subset(mtcars, select=c(mpg,drat))
```
...
permtest <- function( n ) {
  ...
  coefs <- rep( NA, n );
  for( i in 1:n ) {
    perm <- sample(1:nrow(mtcars.small), replace=FALSE)
    mtcars.perm <- mtcars.small;
    # Permute the drat entries.
    mtcars.perm$drat <- mtcars.small[perm,]$drat
    coefs[i] <- coef(lm(mpg ~ drat, data=mtcars.perm))[2];
  }
  return(coefs)
}
